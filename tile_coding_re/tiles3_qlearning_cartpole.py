...
dim_size = 20
tracking_states_static_cart = np.array([list(item) for item in product(*np.array([np.linspace(l, h, dim_size) for l, h in ranges]))])
...
plt.ion()
figsize = (15, 14)
gs_kw = dict(width_ratios=[1, 1], height_ratios=[3, 3, 1, 1])
fig, axd = plt.subplot_mosaic([['static_cart', 'cart_moving_left'],
                               ['cum_rew', 'cum_rew'],
                               ['err', 'err']],
...
ax_val = axd['vals']
ax_sv = axd['sv']
ax_static = axd['static_cart']
ax_cartmovingleft = axd['cart_moving_left']
