...
from tile_coding_re.mc_method.tile_coding import QValueFunction, get_tilings_from_env


def check_constants_py_args():
    from tile_coding_re.sarsa.constants import N_OBS, N_ACT, NB_TILINGS, NB_BINS, LR
    ...
    qvf = QValueFunction(tilings, actions, LR)
...
def heat_map_qtable_size():
    ...
    for n_obs in env_sz_range:
        for n_act in env_sz_range:
            ...
            im = ax.imshow(sizes, norm=LogNorm(), extent=(min(nb_bins_range), max(nb_bins_range), min(nb_tilings_range), max(nb_tilings_range)))
